...
class SparseVDLinear(nn.Module):
    ...
    def reset_parameters(self) -> None:
        """Use the initialization from the paper-author PyTorch example."""
        nn.init.normal_(self.mean_weight, mean=0.0, std=0.02)
    ...
    def log_alpha(self) -> torch.Tensor:
        ...
        return torch.clamp(value, min=-10.0, max=10.0)
    ...
    def posterior_weight_sample(self) -> torch.Tensor:
        ...
        sampled = self.mean_weight + torch.exp(self.log_sigma_weight) * torch.randn_like(
            self.mean_weight
        )
    ...
    def kl_divergence(self) -> torch.Tensor:
        ...
        # This is the negative of the approximate negative-KL expression in
        # Molchanov et al. Additive constants are omitted, as in the authors'
        # minimal PyTorch implementation, because they do not affect gradients.
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.training:
            ...
            output_mean = F.linear(input, self.mean_weight, self.mean_bias)
            output_variance = F.linear(
                input.square(),
                torch.exp(2.0 * self.log_sigma_weight),
                bias=None,
            )

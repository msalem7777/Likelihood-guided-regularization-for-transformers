...
#%%
# Function to plot radar chart for two hyperparameter levels in a (3, 2) layout
def plot_combined_radar(data_0_1, data_0_5, metrics, methods, training_sizes):
    # Number of variables
    num_vars = len(metrics)
    angles = np.linspace(0, 2 * np.pi, num_vars, endpoint=False).tolist()
    angles += angles[:1]  # Close the radar chart

    fig, axs = plt.subplots(3, 2, figsize=(16, 24), subplot_kw=dict(polar=True))  # (3, 2) layout

    # Loop over training sizes and hyperparameters (0.1 -> left column, 0.5 -> right column)
    for row_idx, training_size in enumerate(training_sizes):
        for col_idx, (hyperparam, data) in enumerate([("0.1", data_0_1), ("0.5", data_0_5)]):
            ax = axs[row_idx, col_idx]
            ax.set_title(f"Training Size: {training_size}\nHyperparameter: {hyperparam}", fontsize=14, pad=20)
    ...
    fig.legend(handles, labels, loc='lower center', fontsize=12, ncol=len(methods), bbox_to_anchor=(0.5, -0.05))
...
plot_combined_radar(data_0_1, data_0_5, metrics, methods, training_sizes)
